...
bool ObjectList::delete_object_by_label(int label)
{
    ...
    /* delete node, set pointer p to the node before the deleted node */
...
int ObjectList::match_centroid(HumanObject *ob, Blob *blob_list, int n_blobs)
{
    ...
    for (int i = 1; i <= n_blobs; i++)
    {
        ...
        if (distance <= 20)
        {
            //printf("match object %d with blob %d centroid with distance %d\n", ob->get_index(), i, distance);
...
int ObjectList::virtual_match_centroid(HumanObject *ob, Blob *blob_list, int n_blobs)
{
    ...
    for (int i = 1; i <= n_blobs; i++)
    {
        ...
        if (distance <= 20)
        {
            //printf("find blob %d on object %d's virtual path with distance %d\n", i, ob->get_index(), distance);
...
int ObjectList::match_centrals(HumanObject *ob, Blob *blob_list, int n_blobs)
{
    ...
    {
        //printf("try matching object %d with central of blob %d\n", ob->get_index(), n);
        ...
        static int16_t central_x[100];
        static int16_t central_y[100];
        int central_num = (blob.n_central_points < 100) ? blob.n_central_points : 100;
...
bool ObjectList::blob_registration(Blob *blob)
{
    ...
    while (p)
    {
        ...
        //printf("try register blob at (%d, %d) to object %d\n", blob_x, blob_y, node->ob->get_index());
        //printf("\t distance %d, size score %d\n", distance, size_score);
        if ((distance < 40) && (size_score < 350) && (size_score > 0))
        {
            //printf("\tregester blob at (%d %d) with size %d to object %d\n", blob_x, blob_y, blob_sz, node->ob->get_index());
...
void ObjectList::matching(Blob *blob_list, int n_blobs)
{
    ...
    while (p)
    {
        ...
        /*static match_fun response_chain[] = {&ObjectList::virtual_match_centroid, &ObjectList::match_centroid, &ObjectList::match_centrals};
        int begin = 1; //p->ob->get_virtual_age() ? 0 : 1;
        int end = 3;   //p->ob->get_virtual_age() ? 1 : sizeof(response_chain) / sizeof(response_chain[0]);
        for (int i = begin; i < end; i++)
        {
            matched_blob_num = (this->*response_chain[i])(p->ob, blob_list, n_blobs);
            if (matched_blob_num != 0)
            {
                break;
            }
        }
    ...
    /* before spawning new object from unmatched blobs,
       check if small blobs could be a fraction from a large blob in previous frame,
       if so, regester it to an existing object*/
