...
void HumanObject::virtual_update(int pos_x, int pos_y, int size)
{
    ...
    last_ = now_;
    //printf("virtual updated at pos (%d, %d) vel (%d, %d)\n", pos_x, pos_y, now_.vel_x, now_.vel_y);
}

void HumanObject::virtual_propagation()
{
    virtual_age_ += 1;
    vrtl_.pos_x = now_.pos_x + now_.vel_x * (virtual_age_ + 1);
    vrtl_.pos_y = now_.pos_y + now_.vel_y * (virtual_age_ + 1);
    //printf("virtual propagated, age: %d, virtual pos: (%d, %d)\n", virtual_age_, vrtl_.pos_x, vrtl_.pos_y);
}
